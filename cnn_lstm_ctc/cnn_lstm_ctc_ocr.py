...
def convnet_layers(images_pl, seqlen_pl, mode):
  ...
  with tf.name_scope('convnet'):
    ...
    conv1 = conv_layer(images_pl, [64, 3, 'same', 'conv1', False], training)  # 120,45
    conv2 = conv_layer(conv1, [64, 3, 'same', 'conv2', True], training)       # 120,45
    pool2 = pool_layer(conv2, 2, [2, 2], 'valid', 'pool2')                    # 60,22
    conv3 = conv_layer(pool2, [128, 3, 'same', 'conv3', False], training)     # 60,22
    conv4 = conv_layer(conv3, [128, 3, 'same', 'conv4', True], training)      # 60,22
    pool4 = pool_layer(conv4, 2, [1, 2], 'valid', 'pool4')                    # 59,11
    conv5 = conv_layer(pool4, [256, 3, 'same', 'conv5', False], training)     # 59,11
    conv6 = conv_layer(conv5, [256, 3, 'same', 'conv6', True], training)      # 59,11
    pool6 = pool_layer(conv6, 2, [1, 2], 'valid', 'pool6')                    # 58,5
    conv7 = conv_layer(pool6, [512, 3, 'same', 'conv7', False], training)     # 58,5
    conv8 = conv_layer(conv7, [512, 3, 'same',  'conv8', True], training)     # 58,5
    pool8 = pool_layer(conv8, [1, 5], [1, 5], 'valid', 'pool8')               # 58,1
    features = tf.squeeze(pool8, axis=2, name='features') # squeeze row dim
