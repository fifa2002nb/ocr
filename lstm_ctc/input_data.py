...
class DataIterator:
  ...
  def __init__(self, data_dir, fill_labels=False):
    ...
    for root, sub_folder, file_list in os.walk(data_dir):
      for file_path in file_list:
        ...
        im = cv2.imread(image_name, 0) # [IMAGE_HEIGHT, IMAGE_WIDTH]
  ...
  def get_input_lens(self, sequences):
    lengths = np.asarray([120 for s in sequences], dtype=np.int64)
...
if __name__ == "__main__":
  import input_data
  data_sets = input_data.read_data_sets("/Users/miles/dev/python_workspace/ocr/dataset", fill_labels=False)
